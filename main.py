...
import json
import os
from CRUD import CreateProduto, ReadProduto
...
def login_janela2(login_janela):
    ...
    def abrir_janela_excluir():
        ...
        tk.Label(janela_excluir, text="Exclua o produto", bg="#5a5a5a", fg="white", font=("Arial", 20, "bold")).pack(pady=18)

        frame_box = tk.Frame(janela_excluir, bg="#4d4d4d", padx=20, pady=20)
        frame_box.pack(pady=10)

        tk.Label(frame_box, text="", bg="#4d4d4d", width=3).grid(
            row=0, column=0)  # espaço para radiobutton
        ...
        lista = ListaProdutos(frame_box, estoque.produtos, selecionado)
        ...
        def confirmar_exclusao():
            ...
            if not codigo_sel:
                mensagem_excluir.config(
                    text="Selecione um produto para excluir.", fg="red")
                return

            if codigo_sel in estoque.produtos:
                # remove do dicionário em memória
                del estoque.produtos[codigo_sel]
                estoque.salvar_produtos()        # salva no JSON
                mensagem_excluir.config(
                    text="Produto excluído com sucesso!", fg="lime")
                selecionado.set("")
                ...
                mensagem_excluir.config(
                    text="Produto não encontrado.", fg="red")
                return
        ...
        BotaoVoltar(janela_excluir, text="VOLTAR", bg='#1A73E8', fg='white', font=("Arial", 12),
    ...
    # Exibe os produtos usando a classe ReadProduto
    ...
    def abrir_janela_edicao():
        ...
        tk.Label(janela_editar, text="Edite o produto", bg="#5a5a5a", fg="white", font=("Arial", 20, "bold")).place(relx=0.5, y=40, anchor="center")
        ...
        for i, (codigo, produto) in enumerate(estoque.produtos.items()):
            tk.Label(janela_editar, text=codigo, bg="#5a5a5a",
                     fg="white", font=("Arial", 11)).place(x=60, y=140 + i * 50)
            tk.Label(janela_editar, text=produto["nome"], bg="#5a5a5a", fg="white", font=(
                "Arial", 11)).place(x=250, y=140 + i * 50)
            tk.Label(janela_editar, text=produto["adicao"], bg="#5a5a5a", fg="white", font=(
                "Arial", 11)).place(x=440, y=140 + i * 50)
        ...
        BotaoVoltar(janela_editar, text="VOLTAR", bg='#1A73E8', fg='white',font=("Arial", 14, "bold")).place(x=660, y=430)
    ...
    def editar_produto(codigo, janela_editar):
        ...
        def confirmar_edicao():
            ...
            if not all([nome, qtd, validade, adicao, fornecedor]):
                mensagem.config(
                    text="Preencha todos os campos obrigatórios!", fg="red")
                return

            if not (validade.isdigit() and len(validade) == 8 and adicao.isdigit() and len(adicao) == 8):
                mensagem.config(
                    text="As datas devem ter 8 dígitos (ddmmaaaa)!", fg="red")
                return
